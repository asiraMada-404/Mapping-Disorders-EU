...
function getColorScale() {
    return [
        'step', ['coalesce', ['get', 'Drug'], 0],
        COLOR_SCALE.low, 4, COLOR_SCALE.mediumLow, 7, COLOR_SCALE.medium, 10, COLOR_SCALE.mediumHigh, 13, COLOR_SCALE.high
    ];
}
...
function updateYearDisplay() {
    ...
    // document.getElementById('map-title').textContent = `Drug Use Disorders (${currentYear})`;
}
...
function setupExtrusionInteraction() {
    ...
    map.on('click', ['data-fill', 'data-extrusion'], (e) => {
        ...
        const drug = feature.properties?.Drug || 0;
        ...
        new maplibregl.Popup({ offset: 25 })
            ...
            .setHTML(`
                ...
                <p>Drug Use Disorders (${currentYear}): <h3> ${drug.toFixed(2)} <small>Cases per 1000 people</small> </h3></p>
...
function calculateAverageData() {
    ...
    for (const year of allYears) {
        ...
        for (const feature of data.features) {
            if (feature.properties && feature.properties.Drug !== undefined) {
                sum += feature.properties.Drug;
                count++;
            }
        }
...
function getCountryData(countryName) {
    ...
    for (const year of allYears) {
        ...
        if (feature && feature.properties?.Drug !== undefined) {
            countryData.push(feature.properties.Drug);
        } else {
