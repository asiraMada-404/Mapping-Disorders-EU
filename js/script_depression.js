...
function getColorScale() {
    return [
        'step', ['coalesce', ['get', 'Depress'], 0],
        COLOR_SCALE.low, 25, COLOR_SCALE.mediumLow, 32, COLOR_SCALE.medium, 39, COLOR_SCALE.mediumHigh, 46, COLOR_SCALE.high
    ];
}
...
function updateYearDisplay() {
    ...
    // document.getElementById('map-title').textContent = `Depressive Disorders (${currentYear})`;
}
...
function setupExtrusionInteraction() {
    ...
    map.on('click', ['data-fill', 'data-extrusion'], (e) => {
        ...
        const depress = feature.properties?.Depress || 0;
        ...
        new maplibregl.Popup({ offset: 25 })
            ...
            .setHTML(`
                ...
                <p>Depressive Disorders (${currentYear}): <h3> ${depress.toFixed(2)} <small>Cases per 1000 people</small> </h3></p>
...
function initializeTrendChart() {
    ...
    trendChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Mean Depressive Disorders',
...
function calculateAverageData() {
    ...
    for (const year of allYears) {
        ...
        for (const feature of data.features) {
            if (feature.properties && feature.properties.Depress !== undefined) {
                sum += feature.properties.Depress;
                count++;
            }
        }
...
function getCountryData(countryName) {
    ...
    for (const year of allYears) {
        ...
        if (feature && feature.properties?.Depress !== undefined) {
            countryData.push(feature.properties.Depress);
        } else {
